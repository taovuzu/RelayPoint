...
const RegisterPage = () => {
  ...
  useEffect(() => {
    if (emailRegistrationStep && isSuccess && !registrationToken) {
      setCurrentStep("otp");
      if (!emailData) {
  ...
  }, [emailRegistrationStep, isSuccess, registrationToken, emailData]);
  ...
  const handleEmailRegistration = async (values) => {
    ...
    try {
      ...
      if (result.meta.requestStatus === "fulfilled") {
        ...
        window.localStorage.setItem("emailForOTP", values.email);
        setResendTimer(120);
        message.success(
  ...
  const handleResendOTP = async () => {
    ...
    try {
      if (!emailData) {
        message.error("Email address not found. Please start over.");
  ...
  const handleOTPVerification = async (values) => {
    ...
    try {
      if (!emailData) {
        message.error("Session expired. Please enter your email again.");
        setCurrentStep("email");
        setLoading(false);
        return;
      }
  ...
  const handleUserRegistration = async (values) => {
    ...
    try {
      ...
      if (result.meta.requestStatus === "fulfilled") {
        ...
        message.success("Registration successful! Welcome to RelayPoint!");
        navigate("/dashboard");
      } else {
